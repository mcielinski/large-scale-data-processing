...
def linear_regression(train_ds, test_ds):
    ...
    # stage 2: add vector embedding column
    stage_2 = SQLTransformer(
        statement='SELECT *, udf_vec(post_text_embedding) \
            AS post_text_embedding_vector FROM __THIS__'
    )
    # stage 3: create a vector of all the features required to train model
    stage_3 = VectorAssembler(
        inputCols=['subreddit_name_numeric', 'is_nsfw', 'num_votes',
...
def binary_classification(train_ds, test_ds, classifier=LogisticRegression):
    # stage 1: transform the column 'subreddit_name' to numeric
    stage_1 = StringIndexer(
        inputCol='subreddit_name',
        outputCol='subreddit_name_numeric'
    ...
    stage_2 = SQLTransformer(
        statement='SELECT *, \
            udf_vec(post_text_embedding) AS post_text_embedding_vector, \
            double(is_nsfw) AS is_nsfw_numeric FROM __THIS__'
    )
    # stage 3: create a vector of all the features required to train model
    stage_3 = VectorAssembler(
...
def multiclass_classification(train_ds, test_ds, classifier=LogisticRegression):
    # stage 1: transform the column 'subreddit_name' to numeric
    stage_1 = StringIndexer(
        inputCol='subreddit_name',
        outputCol='subreddit_name_numeric'
    ...
    # stage 3: create a vector of all the features required to train model
    stage_3 = VectorAssembler(
        inputCols=['is_nsfw', 'post_length', 'num_votes',
                   'num_comments', 'num_shares'],
        outputCol='features'
    )
    # stage 4: classifier model
